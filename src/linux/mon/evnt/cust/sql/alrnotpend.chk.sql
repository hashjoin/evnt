...
/*
   NOTE:
      this script requires the following privs granted to MON user:
   
    connect apps...
    grant select on alr_periodic_alerts_view to mon;
    grant select on fnd_concurrent_programs to mon;
    grant select on fnd_concurrent_requests to mon;
*/

select a.application_id
||','||a.alert_id
||','||replace(a.alert_name,',',' ')
